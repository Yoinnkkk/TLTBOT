...
let commands2 = async function (client, command, param1, m, MessageEmbed) {
 ...
 Say - \n Usage: \n $say {text} \n Description: \n Says the specified text\n`); */
        ...
        case "$say":
            ...
            splonk.splice(0,1);
            splonk = splonk.join(' ');
            if (splonk.includes('cock') == false || splonk.includes('dick') == false || splonk.includes('penis') == false || splonk.includes('dildo') == false) {m.channel.send(splonk);} else {m.channel.send(`bro? 🤨`)}
            break;
        default:
